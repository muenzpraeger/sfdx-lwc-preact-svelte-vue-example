...
class List extends d {
  constructor(...args) {
    super(...args);

    _defineProperty(this, "ref", y());
  ...
  render() {
    if (this.state.accounts) {
      const listItems = this.state.accounts.map(account => h("li", {
        style: {
          cursor: "pointer"
        },
      ...
      return h("div", null, this.props.title, h("ul", {
        ref: this.ref
      }, listItems));
    }
...
class App extends d {
  render(props) {
    return h("div", {
      id: "app"
    }, "Data from outer property: ", props.title, h("br", null), h("br", null), h(List, null));
  }
...
function createApp(el, data) {
  const app = h(App, {
    title: data.title
  }); // We're using a component with lifecycles for this example.
